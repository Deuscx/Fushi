...
- **[x] ① 已修复** — `dispatchMangaSelection`（`manga_fushi_page.dart:277`）改成先发起制卡页物化、立即设句并查词、返回前再等物化收尾：查词不再被它挡住，调用方「返回即两者都落地」的语义不变；页面侧既有 `_miningPageGeneration` 代次守卫保证慢的旧物化不会覆盖新点击。提交：`f18c13e6c8`。
