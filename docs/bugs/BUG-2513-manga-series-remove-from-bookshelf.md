...
- **[x] ① 已修复** — 在库态按钮改成「移出漫画书架」（`manga_series_remove_from_bookshelf`），确认框后走书架同一条 `ReaderFushiSource.deleteBook`（DB 行 + 解压目录里已下载章节 + 章节状态 + 下载任务行，`deleteEpubBook` 一个事务级联），删前先取消本书的 OCR 与排队/进行中的下载任务（先停引用再销毁实体）。删完页面**不关**、退回未入库态（条目与章节列表还在手上，可立刻重新加入或换源）。书架经 `_epubBookKeysProvider` 订阅自动重算，不用手动刷。
